# Control plane (hosted web) and Redis

The Next.js app under `apps/web` is the **Locus-style control plane**: users, workspaces, encrypted wallet keys (vault), API keys, and server-side signing on `POST /api/agent/pay`.
